...
export function MemoryDetail({ memory, isOpen, onClose }: MemoryDetailProps) {
  ...
  return (
    <aside
      className={`
        fixed top-20 right-5 w-96 max-h-[calc(100vh-100px)]
      ...
      `}
      style={{
        background: 'linear-gradient(135deg, rgba(0, 217, 255, 0.05) 0%, rgba(123, 104, 238, 0.02) 100%)',
      }}
    >
      {/* 头部 */}
      <div className="p-6 border-b border-gray-800 flex items-center justify-between bg-gray-900/60 rounded-t-2xl">
        ...
        <section>
          ...
          <div className="space-y-2">
            {memory.relations.map((relation, index) => (
              <div
                key={index}
                className="flex items-center p-3 bg-cyan-500/5 border border-gray-800 rounded-md text-sm"
              >
                <span className="text-gray-300">{relation.source}</span>
                <span className="mx-2 text-cyan-400 font-medium">→ {relation.relation} →</span>
        ...
        <section>
          ...
          <div className="flex flex-wrap gap-2">
            {memory.tags.map((tag) => (
              <span
                key={tag.id}
                className="px-3 py-1.5 bg-purple-500/10 border border-purple-500/30 rounded-full text-xs text-purple-400"
              >
                {tag.name}
              </span>
            ))}
